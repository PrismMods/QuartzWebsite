...
- **Move and arrange** — drag elements anywhere, resize from the handles, and multi-select with a marquee or ++shift++-click. **Snap** keeps dragging and resizing aligned to the grid and neighbouring edges, with guides while you move; turn it off for one-pixel placement. Hold ++alt++ while dragging to temporarily reverse the current snapping choice, or ++shift++ to lock the drag to one axis.
...
- **Tabs** — keep several layouts side by side (each with its own **Tab Name**) and switch between them. Drag a tab to reorder it; the saved layout keeps that order. The toolbar carries two strips: **Hand** tabs are your key layouts, and **Foot** tabs hold foot keys, and each strip keeps its own order. One of each can be active at a time, and whichever you aren't editing shows through as a ghost on the canvas so you can line the two up.
- **Foot keys** — the **Foot** strip has its own add and remove buttons. Add builds a foot tab with the number of foot keys you pick, and remove offers **Hide foot keys** to switch them off while keeping the tab, or **Delete this foot tab** to throw it away. In game the foot keys are their own panel, placed beside the hand keys to start with and dragged anywhere in Reorganize mode, with their own **Foot Scale**, **Foot Offset X**, and **Foot Offset Y**. The foot offsets are measured from the hand panel, so moving the Key Viewer carries the foot keys along; **Reset Position** puts them back beside the hand keys. Foot presses light up but don't count toward the total.
...
- **DM Note v2 gradients** — preset gradients carry over to key backgrounds, borders, labels, counters, note rain, and rain glow, including custom colour stops and angles.
